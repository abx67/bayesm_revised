my_rhierMnlRwMixture=function(Data,Prior,Mcmc){
#
# revision history:
...
# 09/22/2014
###################################################################
if(is.null(lgtdata[[1]]$initial_price_state_dropped)){
  print("initial_price_state_dropped is NULL and assign 1 to it")
  for(i in 1:length(lgtdata)) lgtdata[[i]]$initial_price_state_dropped = matrix(1,1,1)
}
if(is.null(lgtdata[[1]]$s1)){
  print("s1 is NULL and assign 1 to it")
  for(i in 1:length(lgtdata)) lgtdata[[i]]$s1 = 1
}
draws =  my_rhierMnlRwMixture_rcpp(lgtdata, Z,
